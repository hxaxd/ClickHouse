...
void KeeperLSMTNodesStorage::prepareImpl(std::string_view path, UncommittedNodeRef && node, KeeperStagingTransaction & staging, ACLId old_acl_id, int64_t ephemeral_owner, bool has_ttl)
{
    state.throttleWrite();
    ...
    staging.deltas.emplace_back(std::string{path}, staging.zxid, LSMTDelta {new_ref, node.node, node.ref, old_digest, old_acl_id, ephemeral_owner, has_ttl});
}
    ...
    KeeperStagingTransaction & staging)
{
    ...
    prepareImpl(path, std::move(node), staging, old_acl_id, new_stats.getEphemeralOwner(), new_stats.isTTL());
}
    ...
    std::string_view data, KeeperStagingTransaction & staging)
{
    ...
    prepareImpl(path, std::move(node), staging, old_acl_id, stats.getEphemeralOwner(), stats.isTTL());
}
    ...
    std::string_view path, UncommittedNodeRef && node, KeeperStagingTransaction & staging)
{
    ...
    node.node.stats = {};
    prepareImpl(path, std::move(node), staging, old_acl_id, ephemeral_owner, has_ttl);
}
